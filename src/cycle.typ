...
#let prepare-path(body, stroke: auto, fill: none) = context {
  
  set path(stroke: merge-strokes(style.stroke, style.fill), fill: merge-fills(fill, style.fill))
  set path(stroke: stroke) if stroke != auto
  body
}
